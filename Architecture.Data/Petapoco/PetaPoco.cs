...
#pragma warning disable 1066,1570,1573,1591
    ...
    public class Database : IDatabase
    {
        ...
        /// <summary>
        ///     Perform Sql Bulk Insert
        /// </summary>
        /// <typeparam name="T">Entity Model</typeparam>
        /// <param name="collection">List of Entity</param>
        /// <remarks>
        ///     This is Custom Function we have created.
        /// </remarks>
        public void BulkInsertRecords<T>(IEnumerable<T> collection)
        {
            try
            {
                ...
                using (var cmd = CreateCommand(_sharedConnection, ""))
                {
                    ...
                    List<string> cols = (from c in pd.QueryColumns select tableName + "." + _provider.EscapeSqlIdentifier(c)).ToList();
                    ...
                    foreach (var poco in collection)
                    {
                        ...
                        foreach (var i in pd.Columns)
                        {
                            if (i.Value.ResultColumn)
                            {
                                cols.Remove(tableName + "." + _provider.EscapeSqlIdentifier(i.Key));
                                continue;
                            }
                            ...
                            {
                                if (i.Value.PropertyInfo.PropertyType == typeof(DateTime) || i.Value.PropertyInfo.PropertyType == typeof(Nullable<DateTime>))
                                {
                                    DateTime value = Convert.ToDateTime(i.Value.GetValue(poco));
                                    if (value < new DateTime(1900, 1, 1))
                                    {
                                        value = new DateTime(1900, 1, 1);
                                    }
                                    values.Add(string.Format("'{0}'", value.ToString("yyyy-MM-dd HH:mm:ss")));
                                }
                                else
                                {
                                    values.Add(string.Format("'{0}'",
                                        i.Value.GetValue(poco).ToString().Replace("'", "''").Replace("@", "@@")));
                                }
                            }
                            else
                            {
                    ...
                    var sql = string.Format("INSERT INTO {0} ({1}) VALUES {2}", tableName, string.Join(",", cols), string.Join(", ", pocoValues));
                    cmd.CommandText = sql;
                    cmd.ExecuteNonQuery();
                }
            }
        ...
        #region operation: Get Result in Data Set
        /// <summary>
